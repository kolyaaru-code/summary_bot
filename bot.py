...
def generate_never_phrase(players: list, chat_context: str = "") -> str:
    ...
    prompt = f"""
Ты придумываешь фразы для игры "Я никогда не" для компании друзей.

ИГРОКИ (знай их имена, но НЕ вставляй их в фразу): {players_str}

ПРАВИЛА:
- Фраза начинается со слов "Я никогда не"
- Тема — реальные жизненные ситуации и приключения: путешествия, экстрим, еда, алкоголь, отношения, работа, деньги, страхи, неловкие моменты, безумные поступки
- Фраза должна быть универсальной — про действие или опыт, НЕ про конкретного человека из чата
- Провокационно и смешно — чтобы половина хотела признаться
- Разнообразие: не только про алкоголь, чередуй темы
- Никаких имён игроков в самой фразе

ПРИМЕРЫ ХОРОШИХ ФРАЗ:
- Я никогда не прыгал с парашютом
- Я никогда не ел насекомых
- Я никогда не уходил с работы посреди дня просто потому что захотел
- Я никогда не врал на собеседовании
- Я никогда не просыпался в чужом городе не помня как туда попал
- Я никогда не тратил больше 10к за один вечер
- Я никогда не дрался по-настоящему
...
async def cmd_help(message: types.Message):
    help_text = (
        ...
        "    /neverstop - останавливают игру «Я никогда не»"
